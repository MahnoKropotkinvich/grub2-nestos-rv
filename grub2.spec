...
elif [ -f %{grub_version_dir}/user.cfg ]; then
    if grep -q '^GRUB_PASSWORD=' %{grub_version_dir}/user.cfg ; then
	sed -i 's/^GRUB_PASSWORD=/GRUB2_PASSWORD=/' \
	    %{grub_version_dir}/user.cfg
    fi
elif [ -f /etc/grub.d/01_users ] && \
	grep -q '^password_pbkdf2 root' /etc/grub.d/01_users ; then
    if [ -f %{grub_version_dir}/grub.cfg ]; then
...
GRUB_VERSION_DIR=%{grub_version_dir}
...
if test -f ${GRUB_VERSION_DIR}/grub.cfg; then
    if (((grep -q "configfile" ${GRUB_VERSION_DIR}/grub.cfg && grep -q "root-dev-only" ${GRUB_VERSION_DIR}/grub.cfg) || grep -q "source" ${GRUB_VERSION_DIR}/grub.cfg) && ! grep -q "# It is automatically generated by grub2-mkconfig using templates" ${GRUB_VERSION_DIR}/grub.cfg); then
        # place configuration and EFI file into EFI_HOME before exiting
        cp -af ${GRUB_VERSION_DIR}/grub*.* ${EFI_HOME}
...
cat << EOF > ${GRUB_VERSION_DIR}/grub.cfg.stb
...
if test -f ${GRUB_VERSION_DIR}/grubenv; then
    cp -a ${GRUB_VERSION_DIR}/grubenv ${GRUB_VERSION_DIR}/grubenv.rpmsave
    mv --force ${GRUB_VERSION_DIR}/grubenv ${GRUB_HOME}/grubenv
fi

mv ${GRUB_VERSION_DIR}/grub.cfg.stb ${GRUB_VERSION_DIR}/grub.cfg

# place configuration and EFI file into EFI_HOME
cp -af ${GRUB_VERSION_DIR}/grub*.* ${EFI_HOME}
...
%{_bindir}/grub2-mount
%attr(0644,root,root) %config(noreplace) /%{_prefix}/dnf/protected.d/grub2-tools-minimal.conf
...
%changelog
* Sat Feb 1 2025 Marta Lewandowska <mlewando@redhat.com> 2.12-22
- unification of bootloader updates, phase 1
